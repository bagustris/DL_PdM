...
dropout_rate = 0.2  # default is 0.2
FINAL_DIM = 900


def build_BILSTM(timesteps, data_dim, hidDim=[100, 140]):
    model = Sequential()
    model.add(Bidirectional(LSTM(hidDim[0], return_sequences=True, 
                            activation='tanh'),
...
def build_CNN(timesteps, data_dim, hidDim=[100, 140]):
    model = Sequential()
    model.add(Conv1D(hidDim[0], 3, activation='tanh',
              input_shape=(timesteps, data_dim)))
...
def build_pre_normalAE(data_dim, X_train, epoch_pretrain=25, hidDim=[100, 140]):
    ...
    ae.fit(X_train, X_train, epochs=epoch_pretrain,
           batch_size=24, shuffle=True, verbose=0)
    model = Sequential()
...
def build_pre_denoiseAE(data_dim, X_train,
                        epoch_pretrain=25,  hidDim=[100, 140]):
    ae = Sequential()
    ae.add(Dropout(0.02, input_shape=(data_dim,)))
    ae.add(Dense(hidDim[0], activation='tanh'))
    ae.add(Dense(hidDim[1], activation='tanh'))
    ae.add(Dense(hidDim[0], activation='tanh'))
    ae.add(Dense(data_dim, activation='linear'))
